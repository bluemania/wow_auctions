...
def what_make() -> None:
    ...
    cols = ['item_id', 'made_from', 'make_pass', 'inv_total_all', 'mean_holding', 'inv_ahm_bag', 'inv_ahm_bank', 'Sell']
    ...
    path = "/Applications/World of Warcraft/_classic_/WTF/Account/396255466#1/SavedVariables/TradeSkillMaster.lua"

    with open(path, "rb") as input_file:
        content = input_file.read()


    start, end = utils.find_attribute_location(content, b'["f@Alliance - Grobbulus@internalData@crafts"]')

    crafting = content[start: end]
    crafting_dict = lua.decode("{" + crafting.decode('ascii') + "}")

    for main_key, item_data in crafting_dict['f@Alliance - Grobbulus@internalData@crafts'].items():
    ...
    for item_name, item_code in materials_list.items():
